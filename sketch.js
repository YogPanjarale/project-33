...
function setup() {
  ...
  ground = new Ground(width/2,height,width,20);
   ...
   for (var k = 0; k <=width; k = k + 80) {
    ...
    for (var j = random(1,5)*10; j <=width; j=j+50) 
    {
    
       plinkos.push(new Plinko(j,75));
    }

    for (var j = random(1,3)*20; j <=width-10; j=j+50) 
    {
    
       plinkos.push(new Plinko(j,175));
    }

     for (var j = random(2,4)*10; j <=width; j=j+50) 
    {
    
       plinkos.push(new Plinko(j,275));
    }

     for (var j = random(2,6)*5; j <=width-10; j=j+50) 
    {
